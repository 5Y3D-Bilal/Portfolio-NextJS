...
const CoustomLink = ({ href, title, className = "" }) => {
  ...
  return (
    <Link href={href} className={`${className} relative group dark:text-white`}>
      ...
      <span
        className={`
            h-[1px] inline-block group-hover:w-full transition-[width] ease duration-300 dark:bg-light  bg-dark absolute left-0 -bottom-0.5 ${
              routor.asPath === href ? "w-full" : "w-0"
            }
            `}
      >
        &nbsp;
    ...
    <header className="w-full px-32 py-8 font-medium flex items-center justify-between">
      ...
      <nav className="flex items-center justify-center flex-wrap">
        ...
        <motion.a
          ...
          whileHover={{ y: -8 }}
          href="https://github.com/5Y3D-Bilal"
          target="_blank"
        >
          <GithubIcon />
        </motion.a>
        <motion.a
          className="w-6 mx-3"
          whileHover={{ y: -8 }}
          href="https://linkedin.com/in/bilal-haider-ab5351299"
          target="_blank"
        >
          <LinkedInIcon />
        </motion.a>
        ...
        <button
          ...
          className="ml-3 flex items-center justify-center rounded-full p-1 bg-light border-gray-500 dark:bg-light border-solid border "
        >
          {mode === "dark" ? (
